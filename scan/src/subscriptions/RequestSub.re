...
let parseResolveStatus = x => {
  let decodedX = x |> Js.Json.decodeString |> Belt_Option.getExn;
  switch (decodedX) {
  | "Pending" => Pending
  | "Success" => Success
  | "Failure" => Failure
         ...
         subscription RequestsMiniByDataSource($id: Int!, $limit: Int!, $offset: Int!) {
           raw_requests(
             ...
             request @bsRecord {
               ...
               clientID: client_id
               requestTime: request_time @bsDecoder(fn: "GraphQLParser.timeStoMoment")
               resolveTime: resolve_time @bsDecoder(fn: "GraphQLParser.timeStoMoment")
...
type reporter_t = {operatorAddress: Address.t};
