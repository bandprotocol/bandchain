    ...
    let decode = json => {
      JsonUtils.Decode.{
        ...
        amount: {
          ...
          | "GUANYU38" =>
            json
            |> at(["msg", "reward_amount"], array(string))
            |> Belt.Array.getExn(_, 0)
            |> GraphQLParser.coins
          | "WENCHANG" => json |> at(["msg", "reward_amount"], string) |> GraphQLParser.coins
...
let get = txHash => {
  let (result, _) =
    ApolloHooks.useSubscription(
      ...
      ~variables=
        SingleConfig.makeVariables(~tx_hash=txHash |> Hash.toHex |> (x => "\\x" ++ x), ()),
    );
  let%Sub x = result;
