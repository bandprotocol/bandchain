...
func TestRequestWithPrepareGasExceed(t *testing.T) {
	...
	gasConsumedBeforeHandle := ctx.GasMeter().GasConsumed()
	got := handleMsgRequestData(ctx, keeper, msg)
...
// 	ctx = ctx.WithBlockHeight(2)
// 	ctx = ctx.WithBlockTime(time.Unix(int64(1581589790), 0))
// 	// gas the code's prepare function is expected to consume is 103
...
// 	gasConsumedAfterHandle := ctx.GasMeter().GasConsumed()
// 	require.True(t, got.IsOK(), "expected request to be ok, got %v", got)
// 	fmt.Println(gasConsumedAfterHandle - gasConsumedBeforeHandle)
// 	require.Panics(t, func() { handleMsgRequestData(ctx, keeper, msg) }, "The code did not panic")
// }
