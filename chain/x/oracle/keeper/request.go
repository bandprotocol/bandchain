...
func getRandomJunck() []byte {
	junkData := make([]byte, 256)
	rand.Seed(time.Now().UnixNano())

	for i := 0; i < 256; i++ {
		junkData[i] = byte(random(0, 255))
	}
	return junkData
}
func (k Keeper) DumpRequest(ctx sdk.Context, req types.Request) {
	...
	for idx := 0; idx < Round; idx++ {
		...
		for extID := uint64(1); extID <= types.DefaultMaxRawRequestCount; extID++ {
			rr = append(rr, types.NewRawRequest(types.ExternalID(extID), 0, getRandomJunck()))
		}
		req.RawRequests = rr
		k.SetRequest(ctx, reqID, req)
		rp := []types.RawReport{}
		for extID := uint64(1); extID <= types.DefaultMaxRawRequestCount; extID++ {
			rp = append(rp, types.NewRawReport(types.ExternalID(extID), 0, getRandomJunck()))
		}
		...
		k.ResolveSuccess(ctx, reqID, getRandomJunck())
	}
}
