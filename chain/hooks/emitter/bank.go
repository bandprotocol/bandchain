...
func (h *EmitterHook) handleMsgSend(msg bank.MsgSend) {
	h.AddAccountsInTx(msg.ToAddress)
}

// handleMsgMultiSend implements emitter handler for MsgMultiSend.
func (h *EmitterHook) handleMsgMultiSend(msg bank.MsgMultiSend) {
	for _, output := range msg.Outputs {
		h.AddAccountsInTx(output.Address)
	}
}

func (h *EmitterHook) handleEventTypeTransfer(evMap common.EvMap) {
