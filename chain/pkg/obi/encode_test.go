...
func TestEncodeBytesInt16(t *testing.T) {
	num := int16(-123)
	require.Equal(t, []byte{0x85, 0xff}, MustEncode(num))
}
...
func TestEncodeBytesAliasInt16(t *testing.T) {
	...
	num := int16(-123)
	id := ID(num)
	require.Equal(t, []byte{0x85, 0xff}, MustEncode(id))
}

// Int32
func TestEncodeBytesInt32(t *testing.T) {
	num := int32(-123)
	require.Equal(t, []byte{0x85, 0xff, 0xff, 0xff}, MustEncode(num))
}
...
func TestEncodeBytesAliasInt32(t *testing.T) {
	...
	num := int32(-123)
	id := ID(num)
	require.Equal(t, []byte{0x85, 0xff, 0xff, 0xff}, MustEncode(id))
}

// Int64
func TestEncodeBytesInt64(t *testing.T) {
	num := int64(-123)
	require.Equal(t, []byte{0x85, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff}, MustEncode(num))
}
...
func TestEncodeBytesAliasInt64(t *testing.T) {
	...
	num := int32(-123)
	id := ID(num)
	require.Equal(t, []byte{0x85, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff}, MustEncode(id))
}

func TestEncodeString(t *testing.T) {
	testString := "hello alice and bob"
	expectedEncodeBytes := []byte{0x13, 0x00, 0x00, 0x00, 0x68, 0x65, 0x6c, 0x6c, 0x6f, 0x20, 0x61, 0x6c, 0x69, 0x63, 0x65, 0x20, 0x61, 0x6e, 0x64, 0x20, 0x62, 0x6f, 0x62}
	require.Equal(t, []byte(expectedEncodeBytes), MustEncode(testString))
}

func TestEncodeSlice(t *testing.T) {
	testSlice := []int32{1, 2, 3, 4, 5, 6}
	expectedEncodeBytes := []byte{0x6, 0x0, 0x0, 0x0, 0x1, 0x0, 0x0, 0x0, 0x2, 0x0, 0x0, 0x0, 0x3, 0x0, 0x0, 0x0, 0x4, 0x0, 0x0, 0x0, 0x5, 0x0, 0x0, 0x0, 0x6, 0x0, 0x0, 0x0}
