...
func decodeImpl(data []byte, v interface{}) ([]byte, error) {
	...
	switch ev.Kind() {
	case reflect.Uint8:
		val, rem, err := DecodeU8(data)
		ev.SetUint(uint64(val))
		return rem, err
	case reflect.Uint16:
		val, rem, err := DecodeU16(data)
		ev.SetUint(uint64(val))
		return rem, err
	case reflect.Uint32:
		val, rem, err := DecodeU32(data)
		ev.SetUint(uint64(val))
		return rem, err
	case reflect.Uint64:
		val, rem, err := DecodeU64(data)
		ev.SetUint(uint64(val))
		return rem, err
	case reflect.Int8:
		val, rem, err := DecodeI8(data)
		ev.SetInt(int64(val))
		return rem, err
	case reflect.Int16:
		val, rem, err := DecodeI16(data)
		ev.SetInt(int64(val))
		return rem, err
	case reflect.Int32:
		val, rem, err := DecodeI32(data)
		ev.SetInt(int64(val))
		return rem, err
	case reflect.Int64:
		val, rem, err := DecodeI64(data)
		ev.SetInt(int64(val))
		return rem, err
	case reflect.String:
...
// DecodeU16 decodes the input bytes into `uint8` and returns the remaining bytes.
...
// DecodeU16 decodes the input bytes into `uint16` and returns the remaining bytes.
...
// DecodeU32 decodes the input bytes into `uint32` and returns the remaining bytes.
func DecodeU32(data []byte) (uint32, []byte, error) {
	if len(data) < 4 {
		return 0, nil, errors.New("obi: out of range")
	}
...
// DecodeU64 decodes the input bytes into `uint64` and returns the remaining bytes.
...
// DecodeI8 decodes the input bytes into `uint64` and returns the remaining bytes.
func DecodeI8(data []byte) (int8, []byte, error) {
	unsigned, rem, err := DecodeU8(data)
	return int8(unsigned), rem, err
}

// DecodeI16 decodes the input bytes into `uint64` and returns the remaining bytes.
func DecodeI16(data []byte) (int16, []byte, error) {
	unsigned, rem, err := DecodeU16(data)
	return int16(unsigned), rem, err
}

// DecodeI32 decodes the input bytes into `uint64` and returns the remaining bytes.
func DecodeI32(data []byte) (int32, []byte, error) {
	unsigned, rem, err := DecodeU32(data)
	return int32(unsigned), rem, err
}

// DecodeI64 decodes the input bytes into `uint64` and returns the remaining bytes.
func DecodeI64(data []byte) (int64, []byte, error) {
	unsigned, rem, err := DecodeU64(data)
