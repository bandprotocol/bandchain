...
fn __return(output: &[u8]) -> u64 {
    ...
    println!("{}", (loc as u32) as u64);
    let x = unsafe { std::slice::from_raw_parts(loc as *const u8, sz) };
    println!("read at {}, size {}", ((loc as *const u8) as u32), sz);
    ((sz as u64) << 32) | ((loc as u32) as u64)
}

fn __read_data<T>(ptr: u64) -> &'static [T] {
    println!("{} size is {}", ((ptr & ((1 << 32) - 1)) as u32) as u64, (ptr >> 32) as usize);
    // unsafe { std::slice::from_raw_parts((ptr & ((1 << 32) - 1)) as *const T, (ptr >> 32) as usize) }
    unsafe { std::slice::from_raw_parts(((ptr & ((1 << 32) - 1)) as u32) as *const T, 1) }
}
