  ...
  "chain.genesis": function() {
    ...
    if (chain && chain.readGenesis) {
          ...
          for (m in msg) {
            if (msg[m].type == "cosmos-sdk/MsgCreateValidator") {
              ...
              let validator = {
                ...
                voting_power: Math.floor(
                  parseInt(msg[m].value.value.amount) /
              ...
              validator.accpub = Meteor.call(
                "pubkeyToBech32",
                validator.pub_key,
                Meteor.settings.public.bech32PrefixAccPub
              );
              validator.operator_pubkey = Meteor.call(
                "pubkeyToBech32",
                validator.pub_key,
                Meteor.settings.public.bech32PrefixValPub
              );
      ...
      if (
        ...
        genesis.app_state.staking.validators.length > 0
      ) {
        ...
        for (let v in genValidatorsSet) {
          ...
          validator.delegator_address = Meteor.call(
            "getDelegator",
            genValidatorsSet[v].operator_address
          );
